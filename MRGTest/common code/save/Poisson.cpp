...
void possionImageEditing_one_channel(TensorByte & ans, const TensorInt & src, const TensorInt & dst, const TensorByte & msk)
{
...
void Poisson::possionImageEditing(TensorV3i & src_pixel, IplImage * dst_img)
{
	if(!dst_img) return;
	...
	for(int i = 0;i<height;i++)
	{
		...
		{	
			...
			LibIV::CppHelpers::Global::imgDataRGB(r,g,b,dst_img,j,i);
	...
	for(int i = 0;i<height;i++)
	{
		for(int j = 0;j<width;j++)
			if(msk(j,i))
				LibIV::CppHelpers::Global::setImgDataRGB(ans_r(j,i),ans_g(j,i),ans_b(j,i),dst_img,j,i);
		//setImgDataRGB(255,255,255,dst_img,j,i);
	}
}
